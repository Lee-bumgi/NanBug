...
//    Serial.println(measure.RangeMilliMeter);
    if(measure.RangeStatus != 4 & measure.RangeMilliMeter < 300 & measure.RangeMilliMeter > 50 & response_1 != "" ) {  
  //    Serial.print("Distance (mm): "); 
      if (flag == 0){
        Serial.println(measure.RangeMilliMeter);
        for (int j = 0; j<3; j++){
          response_2 += number_list_1[j];
        }
        int number_list_1[]={0,0,0};
        for (int j = 0; j<3; j++){
          response_2 += number_list_2[j];
        }
        int number_list_2[]={0,0,0};
        for (int j = 0; j<3; j++){
          response_2 += number_list_3[j];
        }
        int number_list_3[]={0,0,0};
//        if (response_1==""){
//          response_1="000000000";
//        }
        Serial.println("1 : "+response_1);
        Serial.println("2 : "+response_2);
        flag = 1;
        response_1 = ""; 
        response_2 = "";
        delay(1000);
